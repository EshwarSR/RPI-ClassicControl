...
OUTPUT_DIR = os.path.join('plots', 'ICC', 'InvertedPendulum', 'net-arch')
os.makedirs(OUTPUT_DIR, exist_ok=True)
OUTPUT_FILENAME = os.path.join(
    OUTPUT_DIR, 'performance_comparison_inverted_pendulum_styled-FINAL-FINAL.png'
)
OUTPUT_FILENAME_PDF = os.path.join(
    OUTPUT_DIR, 'performance_comparison_inverted_pendulum_styled-PDF.pdf'
)
...
def generate_table_data(arch_dirs):
    ...
    csv_filename = os.path.join(OUTPUT_DIR, 'results_summary_pendulum.csv')
    df.to_csv(csv_filename)
    print(f"\nTable saved to {csv_filename}")

    latex_filename = os.path.join(OUTPUT_DIR, 'results_summary_pendulum.tex')
