    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        for _ in range(gradient_steps):
            ...
            with th.no_grad():
                ...
                # Get max actions based on the main network
                max_actions = self.q_net(replay_data.next_observations).argmax(dim=1, keepdim=True)
                # Corresponding Q-values from target network
                next_q_values = th.gather(next_q_values, dim=1, index=max_actions)
                # 1-step TD target
